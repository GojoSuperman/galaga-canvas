...
test('경과 시간만큼 update를 고정 step으로 반복 호출한다', () => {
  ...
  const loop = createLoop({
    update: (dt) => updates.push(dt), render: () => {}, step: 0.1, maxFrameTime: 1,
  });
  loop.tick(0);
  loop.tick(0.35);
  ...
  return {
    update(dt) {
      ...
      if (phase === 'respawning') {
        ...
        // (플레이어 탄까지 갱신해야 한다 — 빠뜨리면 죽는 순간 날아가던 내 탄이 공중에 얼어붙는다.)
        formation.update(dt);
        spawner.update(dt);
...
# 확장: 10스테이지 + 보스 2회 + 새 적 패턴

> 2026-07-14 추가. 사용자가 5스테이지를 클리어한 뒤 "5개 더"를 요청했고, 보스 배치(5·10 두 번)와 난이도 상승 방식(새 적 패턴 추가)을 선택했다.

## 확장 설계

- **스테이지 10개.** 5스테이지 = 중간 보스, 10스테이지 = 최종 보스(더 강함).
- **새 적 `elite`** — 체력 3, 급강하 시 **플레이어를 조준해서** 사격한다. 기존 적은 전부 아래로 직선 탄만 쏘므로, elite는 "피하기"가 아니라 "위치 선정"을 요구하는 질적으로 다른 위협이 된다.
- **새 진입 경로 2종** (`spiralLeft` / `spiralRight`) — 화면을 크게 가로지르는 나선. 후반 스테이지의 등장 연출을 바꾼다.
- 보스는 **티어로 파라미터화**해 같은 3페이즈 로직을 두 번 재사용한다.

## Task 15 (수정): 보스를 티어로 파라미터화

Task 15의 원래 내용은 그대로 두되, 아래를 반영한다.

`js/game/boss.js`:

```js
/**
 * 보스 티어별 능력치. 1 = 5스테이지 중간 보스, 2 = 10스테이지 최종 보스.
 * 플레이테스트 반영(2026-07-14): 체력을 2/3로 낮추고, 대신 졸개를
 * 더 자주(summonInterval 감소) · 더 많이(summonCount 증가) 소환한다.
 */
export const BOSS_TIERS = {
  1: { maxHp: 80,  speedScale: 1.0,  bulletSpeedScale: 1.0,  spreadBonus: 0, summonInterval: 5, summonCount: 2 },
  2: { maxHp: 147, speedScale: 1.35, bulletSpeedScale: 1.25, spreadBonus: 2, summonInterval: 4, summonCount: 3 },
};
```

- `createBoss(game, enemyBullets, tier = 1)` — `BOSS_TIERS[tier]`로 최대 체력을 정하고, `BOSS_PHASE_CONFIG`의 `moveSpeed`·`bulletSpeed`에 스케일을 곱하고, `spreadCount`에 `spreadBonus`를 더한다. `summonTimer`는 `BOSS_TIERS[tier].summonInterval`로 초기화·리셋되고, `summonCount`를 보스 객체에 노출해 `playScene`이 한 번에 몇 마리를 소환할지 알 수 있게 한다.
- `hpRatio`는 `hp / BOSS_TIERS[tier].maxHp` — 페이즈 전이 기준(66% / 33%)은 티어와 무관하게 동일하다.
- HP 게이지 위 라벨은 티어에 따라 `BOSS` / `FINAL BOSS`로 표시한다.
...
스테이지 데이터는 `isBoss: true`인 스테이지에 `bossTier: 1 | 2`를 함께 갖는다. `playScene`은 `createBoss(game, enemyBullets, stage().bossTier)`로 생성하고, 격파 시 `score += BOSS_SCORE[stage().bossTier]`를 더한다.

---

## Task 15b: 새 적 `elite` + 나선 진입 경로

**Files:**
- Modify: `js/gfx/pixels.js` (elite 스프라이트 추가)
- Modify: `js/game/paths.js` (`spiralLeft` / `spiralRight` 추가)
- Modify: `js/game/enemy.js` (`elite` 타입 추가)
- Modify: `js/game/score.js` (`elite` 점수 추가)
- Modify: `js/scenes/playScene.js` (조준탄 분기)
- Test: `test/paths.test.js`, `test/enemy.test.js`, `test/score.test.js` 확장

**Interfaces:**
- Consumes: `aimedShot(originX, originY, targetX, targetY, speed)` — Task 15에서 `js/game/boss.js`에 이미 만든 것을 재사용한다.
- Produces:
  - `ENTRY_PATHS.spiralLeft`, `ENTRY_PATHS.spiralRight`
  - `ENEMY_TYPES.elite = { hp: 3, sprite: 'elite', w: 15, h: 15, aimed: true }`
  - `ENEMY_BASE_SCORE.elite = 250`

- [ ] **Step 1: elite 스프라이트 추가** — `js/gfx/pixels.js`
...
- [ ] **Step 2: 나선 진입 경로 테스트 추가** — `test/paths.test.js`

기존 "모든 진입 경로는 목표 슬롯에서 끝난다" / "진입 경로는 화면 밖에서 시작한다" 테스트는 `ENTRY_PATHS`를 순회하므로 **새 경로를 자동으로 검사한다.** 추가로 나선이 화면 안에 머무는지 확인하는 테스트를 넣는다:
...
Run: `npm test`
Expected: FAIL — `spiralLeft`가 `ENTRY_PATHS`에 없어 `build is not a function`

- [ ] **Step 4: 나선 경로 구현** — `js/game/paths.js`의 `ENTRY_PATHS`에 추가
...
- [ ] **Step 5: elite 적 타입 테스트 추가** — `test/enemy.test.js`
...
Run: `npm test`
Expected: FAIL — `ENEMY_TYPES.elite`가 `undefined`

- [ ] **Step 7: elite 타입 구현**

`js/game/enemy.js`의 `ENEMY_TYPES`에 추가:

```js
  // 엘리트 — 단단하고, 급강하 중 플레이어를 조준해서 쏜다.
  elite: { hp: 3, sprite: 'elite', w: 15, h: 15, aimed: true },
```

`js/game/score.js`의 `ENEMY_BASE_SCORE`에 추가:

```js
  elite: 250,
```

- [ ] **Step 8: 조준탄 발사 분기** — `js/scenes/playScene.js`

import에 `aimedShot`을 추가한다 (Task 15에서 `boss.js`에 이미 있다):

```js
import { createBoss, aimedShot } from '../game/boss.js';
import { ENEMY_STATE, ENEMY_TYPES } from '../game/enemy.js';
```

`enemyShooting(dt)`의 탄 생성 부분을 다음으로 교체한다:
...
Run: `npm test`
Expected: PASS — 새 테스트 6개 포함 전부 통과

- [ ] **Step 10: 커밋**

```bash
git add js/gfx/pixels.js js/game/paths.js js/game/enemy.js js/game/score.js js/scenes/playScene.js test/
git commit -m "feat: 엘리트 적(체력3·조준탄)과 나선 진입 경로 2종"
```
...
**Files:**
- Modify: `js/stages/stages.js` (스테이지 5개 추가 + 보스 티어)
- Modify: `test/stages.test.js` (검증 규칙 확장)

- [ ] **Step 1: 검증 테스트 확장** — `test/stages.test.js`

기존 테스트 중 스테이지 수와 보스 위치를 검사하는 두 개를 교체하고, 티어 검사를 추가한다:
...
난이도 단조 증가 테스트는 **보스 스테이지를 건너뛰고** 일반 스테이지끼리만 비교하도록 바꾼다 (보스전은 편대 난이도 곡선의 일부가 아니다):
...
나머지 검증 테스트(적 타입·경로 유효성, 슬롯 중복, 행·열 범위)는 `STAGES`를 순회하므로 **새 스테이지를 자동으로 검사한다.** 손댈 필요가 없다.

- [ ] **Step 2: 테스트 실패 확인**

Run: `npm test`
Expected: FAIL — `STAGES.length`가 5이므로 "스테이지는 10개다"부터 실패

- [ ] **Step 3: 스테이지 5에 보스 티어 부여** — `js/stages/stages.js`

기존 5번째 스테이지 객체에 한 줄 추가:
...
- [ ] **Step 4: 스테이지 6~10 추가** — `js/stages/stages.js`의 `STAGES` 배열 끝에
...
Run: `npm test`
Expected: PASS — 스테이지 검증 테스트가 새 스테이지 5개의 슬롯 중복·타입 오타·난이도 역전을 전부 검사하고 통과한다.

- [ ] **Step 6: 10스테이지 전체 시뮬레이션**

throwaway 스크립트로 10개 스테이지 각각을 스포너로 돌려, 모든 적이 대형에 합류하고 슬롯이 겹치지 않는지 확인한다 (Task 12의 검증과 동일한 방식).

- [ ] **Step 7: 커밋**

```bash
git add js/stages/stages.js test/stages.test.js
git commit -m "feat: 스테이지 6~10 추가 (보스 2회, 엘리트/나선 도입)"
```
